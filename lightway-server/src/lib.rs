...
impl<SA: for<'a> ServerAuth<AuthState<'a>>> AuthAdapter<SA> {
    ...
    pub fn with_rate_limiter(auth: SA, rate_limiter: AuthRateLimiter) -> Self {
        Self {
            auth,
    ...
    for AuthAdapter<SA>
{
    fn authorize(
        ...
        if matches!(authorized, ServerAuthResult::Denied) {
            // Reset rate limit on successful authentication to allow more attempts
            if let Some(limiter) = &self.rate_limiter {
                limiter.reset(&app_state.peer_addr);
            }
            metrics::connection_rejected_access_denied();
        }
        authorized
